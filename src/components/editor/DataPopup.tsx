...
export function DataPopup({ children }: DataPopupProps) {
  ...
  const handleOutputClick = (outputKey: string, stepId: string) => {
    // TODO: Implement output insertion logic
    console.log(`Inserting output: ${outputKey} from step: ${stepId}`)
  }
            ...
            {!selectedStepId ? (
              ...
              <div className="space-y-2">
                <p className="text-sm text-gray-600 mb-3">
                  Drag a data item into a text field or click to insert at the current position.
                </p>
                      ...
                      <div className="border-t border-gray-200 p-3 space-y-2">
                        {step.outputs.map((output) => (
                          <div
                            ...
                            className="flex items-center justify-between p-2 rounded border border-gray-200 hover:bg-blue-50 hover:border-blue-300 transition-colors cursor-pointer"
                          >
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                <span className="text-sm font-medium">{output.outputTitle}</span>
                                <Badge className={`text-xs ${getDataTypeColor(output.outputDataType)}`}>
                                  {output.outputDataType}
                                </Badge>
                              </div>
                              {output.outputDescription && (
                                <p className="text-xs text-gray-500 mt-1">{output.outputDescription}</p>
                              )}
                            </div>
                          </div>
                        ))}
